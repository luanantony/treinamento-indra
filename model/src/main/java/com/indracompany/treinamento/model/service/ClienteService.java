...
public class ClienteService extends GenericCrudService<Cliente, Long, ClienteRepository>{
	  ...
	  public List<ClienteDTO> buscarClientePorNome(String nome){
		  List<Cliente> nomeClientes = repository.findByNomeContaining(nome);
		  
		  if (nomeClientes == null || nomeClientes.isEmpty()) {
			  throw new AplicacaoException(ExceptionValidacoes.ALERTA_NENHUM_REGISTRO_ENCONTRADO);
		  }
		  
		  List<ClienteDTO> retorno2 = new ArrayList<ClienteDTO>();
		  for (Cliente c: nomeClientes) {
			  ClienteDTO dto = new ClienteDTO();
			  dto.setEmail(c.getEmail());
			  dto.setNome(c.getNome());
			  retorno2.add(dto);
